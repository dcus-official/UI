...
local Library = {
    ...
    -- Gradient Accent --
    GradientStart  = Color3.fromRGB(0, 85, 255);
    GradientEnd    = Color3.fromRGB(180, 0, 255);
    GradientRotate = 0;
    ...
    if Library.IsMobile == false then
        Instance.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                -- Drag hide: hide tab area while dragging
                if IsMainWindow then
                    for _, child in ipairs(Instance:GetDescendants()) do
                        if child.Name == "MainSectionOuter" and child:IsA("Frame") then
                            child.Visible = false
                            break
                        end
                    end
...
Library.GradientRegistry = {}  -- { frame, gradient }

function Library:ApplyGradient(frame)
    frame.BackgroundColor3 = Library.GradientStart
    local g = frame:FindFirstChildOfClass("UIGradient")
    if not g then g = Instance.new("UIGradient"); g.Parent = frame end
    g.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Library.GradientStart),
        ColorSequenceKeypoint.new(1, Library.GradientEnd),
    })
    g.Rotation = Library.GradientRotate
    table.insert(Library.GradientRegistry, {frame = frame, gradient = g})
    return g
end

function Library:UpdateAllGradients()
    Library.GradientStart  = Library.GradientStart  or Library.AccentColor
    Library.GradientEnd    = Library.GradientEnd    or Library.AccentColor
    Library.GradientRotate = Library.GradientRotate or 0
    for _, entry in ipairs(Library.GradientRegistry) do
        if entry.updateFunc then
            entry.updateFunc()
        elseif entry.frame and entry.frame.Parent then
            entry.frame.BackgroundColor3 = Library.GradientStart
            entry.gradient.Color = ColorSequence.new({
                ColorSequenceKeypoint.new(0, Library.GradientStart),
                ColorSequenceKeypoint.new(1, Library.GradientEnd),
            })
            entry.gradient.Rotation = Library.GradientRotate
        end
    end
end
...
do
    ...
    function BaseAddonsFuncs:AddColorPicker(Idx, Info)
        ...
        -- Highlight gradient applied below
        Library:ApplyGradient(Highlight)
...
function Library:CreateWindow(...)
    ...
    do
        ...
        WinStroke.Color = Library.GradientStart
        WinStroke.Parent = Inner
        ...
        WinGrad.Rotation = Library.GradientRotate
        WinGrad.Parent = WinStroke

        table.insert(Library.GradientRegistry, {frame = WinStroke, gradient = WinGrad,
            updateFunc = function()
                WinStroke.Color = Library.GradientStart
                WinGrad.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Library.GradientStart),
...
do
        ...
        function Tab:AddGroupbox(Info)
            ...
            local Highlight = Library:Create("Frame", {
                BackgroundColor3 = Library.GradientStart;
                BorderSizePixel = 0;
                Size = UDim2.new(1, 0, 0, 2);
                ZIndex = 5;
                Parent = BoxInner;
