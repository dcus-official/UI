...
local ThemeManager = {} do
	local ThemeFields = { "FontColor", "MainColor", "AccentColor", "BackgroundColor", "OutlineColor", "VideoLink", "GradientStart", "GradientEnd" }
	...
	function ThemeManager:ApplyTheme(theme)
		...
		for idx, col in next, customThemeData or scheme do
			...
			elseif idx == "GradientRotate" then
				self.Library[idx] = tonumber(col) or 0
				if self.Library.Options[idx] then
					self.Library.Options[idx]:SetValue(tonumber(col) or 0)
				end
			elseif idx == "GradientStart" or idx == "GradientEnd" then
				...
				if ok then
					self.Library[idx] = c
					if self.Library.Options[idx] then
						self.Library.Options[idx]:SetValueRGB(c)
					end
				end
			else
	...
	function ThemeManager:ThemeUpdate()
		...
		-- Sync gradient from options if available
		if self.Library.Options and self.Library.Options["GradientStart"] then
			self.Library.GradientStart = self.Library.Options["GradientStart"].Value
		end
		if self.Library.Options and self.Library.Options["GradientEnd"] then
			self.Library.GradientEnd = self.Library.Options["GradientEnd"].Value
		end
		if self.Library.Options and self.Library.Options["GradientRotate"] then
			self.Library.GradientRotate = self.Library.Options["GradientRotate"].Value
		end
	...
	function ThemeManager:SaveCustomTheme(file)
		...
		for _, field in next, ThemeFields do
			if field == "VideoLink" then
				theme[field] = self.Library.Options[field] and self.Library.Options[field].Value or ""
			elseif field == "GradientRotate" then
				theme[field] = self.Library.Options[field] and self.Library.Options[field].Value or 0
			elseif field == "GradientStart" or field == "GradientEnd" then
	...
	function ThemeManager:CreateThemeManager(groupbox)
		...
		groupbox:AddLabel('Gradient Start'):AddColorPicker('GradientStart', {
			Default = self.Library.GradientStart or self.Library.AccentColor,
			Callback = function(v)
				self.Library.GradientStart = v
				if self.Library.UpdateAllGradients then self.Library:UpdateAllGradients() end
			end
		});
		groupbox:AddLabel('Gradient End'):AddColorPicker('GradientEnd', {
			Default = self.Library.GradientEnd or self.Library.AccentColor,
			Callback = function(v)
				self.Library.GradientEnd = v
				if self.Library.UpdateAllGradients then self.Library:UpdateAllGradients() end
			end
		});
		groupbox:AddLabel('Gradient Rotate'):AddSlider('GradientRotate', {
			Default = self.Library.GradientRotate or 0,
			Min = 0, Max = 360, Rounding = 0,
			Callback = function(v)
				self.Library.GradientRotate = v
				if self.Library.UpdateAllGradients then self.Library:UpdateAllGradients() end
			end
		});
		...
		if self.Library.Options.GradientStart then self.Library.Options.GradientStart:OnChanged(UpdateTheme) end
		if self.Library.Options.GradientEnd   then self.Library.Options.GradientEnd:OnChanged(UpdateTheme)   end
		if self.Library.Options.GradientRotate then self.Library.Options.GradientRotate:OnChanged(UpdateTheme) end
	end
