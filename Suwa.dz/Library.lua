...
local WaterMark = [[
	Acrylic | V1.3
	-----------------------------------
	Fix: removed erroneous GuiInset double-offset.
	     ScreenPointToRay already operates in GUI coordinate space.
	Add: Padding setting for per-edge pixel-perfect size control.
]]
print(WaterMark)

Library.AcrylicSettings = {
	Enabled          = true,
	Tag              = "Acrylic",
	Material         = Enum.Material.Glass,
	PartColor        = Color3.fromRGB(248, 248, 248),
	PartTransparency = 0.98,
	PartSize         = 0.22,
...
local Settings = Library.AcrylicSettings

local DepthOfField
local activeCount = 0

local function ensureDepthOfField()
	if DepthOfField and DepthOfField.Parent then return end
	DepthOfField = Instance.new("DepthOfFieldEffect")
	DepthOfField.Name            = "AcrylicDepthOfField"
...
local function releaseDepthOfField()
	activeCount = math.max(0, activeCount - 1)
	if activeCount == 0 and DepthOfField then
		DepthOfField:Destroy()
		DepthOfField = nil
	end
end

local acos, max, pi = math.acos, math.max, math.pi
...
local function DrawTriangle(v1, v2, v3, p0, p1)
	...
	local para   = AB:Dot(AC) / ABmag
	local perp   = math.sqrt(math.max(0, AC.Magnitude ^ 2 - para ^ 2))
	local dif_para = ABmag - para
...
local function getClippedRect(obj)
	local pos  = obj.AbsolutePosition
	local size = obj.AbsoluteSize

	local minX = pos.X - Settings.Padding.Left
	local minY = pos.Y - Settings.Padding.Top
	local maxX = pos.X + size.X + Settings.Padding.Right
	local maxY = pos.Y + size.Y + Settings.Padding.Bottom
	...
	while parent and parent:IsA("GuiObject") do
		if parent.ClipsDescendants then
			...
			minX = math.max(minX, pPos.X)
			minY = math.max(minY, pPos.Y)
			maxX = math.min(maxX, pPos.X + pSize.X)
			maxY = math.min(maxY, pPos.Y + pSize.Y)
		end
...
local AcrylicHandler = {}

function AcrylicHandler:Apply(guiObject: GuiObject)
	if not guiObject:IsDescendantOf(LP) then return end

	local camera = workspace.CurrentCamera
	if not camera then return end
	waitForValidCamera(camera)

	ensureDepthOfField()
	activeCount += 1

	local renderName = "Acrylic_" .. HS:GenerateGUID(false)
	...
	local function getTotalRotation(inst)
		local rot    = 0
		local cursor = inst
		while cursor and cursor:IsA("GuiObject") do
			rot    = rot + cursor.Rotation
			cursor = cursor.Parent
		end
		return rot
	end

	local function setPartsVisible(visible)
		for _, pt in ipairs(parts) do
			if pt and pt.Parent then
				pt.Transparency = visible and Settings.PartTransparency or 1
			end
		end
	end
	...
	local function update()
		...
		local topLeft, bottomRight = getClippedRect(guiObject)

		if not topLeft or not Settings.Enabled then
			if not hidden then
				hidden = true
		...
		local rot = getTotalRotation(guiObject)
		if rot ~= 0 and rot % 180 ~= 0 then
			...
			local function rotatePoint(p)
				local dx = p.X - mid.X
				local dy = p.Y - mid.Y
				return Vector2.new(c * dx - s * dy + mid.X, s * dx + c * dy + mid.Y)
			end
			tl, tr, bl, br = rotatePoint(tl), rotatePoint(tr), rotatePoint(bl), rotatePoint(br)
		end

		local depth = 1 - 0.05 * guiObject.ZIndex
		...
		if not initialized then
			for _, pt in ipairs(parts) do
				pt.Parent       = container
				pt.Transparency = Settings.PartTransparency
				pt.Color        = Settings.PartColor
			end
			initialized = true
		end
		...
		for _, pt in ipairs(parts) do
			if pt and pt.Parent then
				pt.Transparency = hidden and 1 or Settings.PartTransparency
				pt.Color        = Settings.PartColor
				pt.Material     = Settings.Material
			end
		end
	end

	update()
	RS:BindToRenderStep(renderName, Enum.RenderPriority.Camera.Value + 1, update)

	local cleanedUp = false
	local function cleanup()
		if cleanedUp then return end
		cleanedUp = true
		RS:UnbindFromRenderStep(renderName)
		container:Destroy()
		releaseDepthOfField()
		return 1
	end
	...
	CollectionService:GetInstanceRemovedSignal(Settings.Tag):Connect(function(v)
		if v ~= guiObject then return end
		cleanup()
	end)
...
AcrylicHandler.new = function(guiObject)
	return AcrylicHandler:Apply(guiObject)
end

CollectionService:GetInstanceAddedSignal(Settings.Tag):Connect(function(inst)
	AcrylicHandler:Apply(inst)
end)

for _, v in ipairs(CollectionService:GetTagged(Settings.Tag)) do
	AcrylicHandler:Apply(v)
end

Library.AcrylicHandler = AcrylicHandler
...
Library.ThemeFolder = "SuwaUI-Themes"
...
local function SetImageAsync(instance, property, idStr)
	...
	if parsedId:find("rbxassetid://") then
		task.spawn(function()
			...
			if success and result and typeof(result) == "Instance" and result:IsA("Decal") then
...
function Library:CreateWindow(Settings)
	...
	Library.ConfigFolder = Config.ConfigFolder or "SuwaUI-Configs"
	...
	function Funcs:CreateTab(TabName, Two_Column, ImageID)
		...
		local function GetElements(TargetParent)
			...
			function Elements:CreateDropdown(Cfg)
				...
				local function Update(val) 
					Selected = val 
					Title.Text = (Cfg.Name or "Dropdown") .. (Selected and " - " .. tostring(Selected) or "")
					if Cfg.Callback then
						Cfg.Callback(val)
					end
					Library.Flags[Flag] = val
			...
			function Elements:CreatePlayerDropdown(Cfg)
				ElementOrder = ElementOrder + 1
				local Flag = Cfg.Flag or Cfg.Name or "PlayerDropdown_" .. HS:GenerateGUID(false)
				local Expanded = false
				local Selected = nil
				local isUpdating = false

				local SearchBarEnabled = Cfg.SearchBar
				local Drop
				local Title
				local SearchBox
				local Arrow
				...
				local function formatPlayerText(player, textTemplate)
					local result = textTemplate
					result = result:gsub("{DisplayName}", player.DisplayName)
					result = result:gsub("{Username}", player.Name)

					-- (clr:Color3.fromRGB(r,g,b):text)
					result = result:gsub("%(clr:Color3%.fromRGB%((%d+),%s*(%d+),%s*(%d+)%)%:([^%)]+)%)", function(r, g, b, text)
						return string.format('<font color="rgb(%d,%d,%d)">%s</font>', tonumber(r), tonumber(g), tonumber(b), text)
					end)

					-- (clr:Color3.new(r,g,b):text)
					result = result:gsub("%(clr:Color3%.new%(([%d%.]+),%s*([%d%.]+),%s*([%d%.]+)%)%:([^%)]+)%)", function(r, g, b, text)
						return string.format('<font color="rgb(%d,%d,%d)">%s</font>', math.floor(tonumber(r) * 255), math.floor(tonumber(g) * 255), math.floor(tonumber(b) * 255), text)
					end)

					-- {Icon}
					local hasIcon = false
					if result:find("{Icon}") then
						hasIcon = true
						result = result:gsub("{Icon}%s*|%s*", ""):gsub("%s*|%s*{Icon}", ""):gsub("{Icon}", "")
					end
				...
				if SearchBarEnabled then
					...
					SearchBox = Create("TextBox", {
						...
						BackgroundTransparency = 1,
						Text = "",
						PlaceholderText = Cfg.Name or "Select Player...",
						TextColor3 = SelectedTheme.Text,
						Font = Library.GlobalFont,
						TextSize = 14,
						TextXAlignment = Enum.TextXAlignment.Left,
				...
				local function ToggleDropdown(state)
					...
					local H = Expanded and math.min(visibleCount * 32 + 44, 180) or 34
					TS:Create(Drop, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, H)}):Play()
					TS:Create(Arrow, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Rotation = Expanded and 180 or 0}):Play()
					TS:Create(Container, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 1, -34)}):Play()
				end
				...
				local function FilterList(query)
					...
					for _, btn in ipairs(Container:GetChildren()) do
						if btn:IsA("TextButton") then
							local pName = btn:GetAttribute("PlayerName") or ""
							local pDisplayName = btn:GetAttribute("PlayerDisplayName") or ""
							if pName:lower():find(query) or pDisplayName:lower():find(query) then
								btn.Visible = true
								visibleCount = visibleCount + 1
							else
								btn.Visible = false
					...
					if Expanded then
						local H = math.min(visibleCount * 32 + 44, 180)
						TS:Create(Drop, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, H)}):Play()
					end
				end
				...
				local function Update(p)
					Selected = p
					local displayStr = p.DisplayName
					local formattedTitle, _ = formatPlayerText(p, Cfg.Text or "{DisplayName} (@{Username})")
					if SearchBarEnabled then
						isUpdating = true
						SearchBox.Text = ""
						isUpdating = false
						SearchBox.PlaceholderText = (Cfg.Name or "Dropdown") .. " - " .. displayStr
					else
						Title.Text = (Cfg.Name or "Dropdown") .. " - " .. formattedTitle
					end
					if Cfg.Callback then
						Cfg.Callback({
							Username = p.Name,
				...
				local function RefreshList()
					...
					local autoupdate = Cfg.Autoupdate or {}
					local whitelist = autoupdate.whitelist or {}
					local blacklist = autoupdate.blacklist or {}
					...
					for _, p in ipairs(playersList) do
						...
						if #whitelist > 0 then
							...
							for _, w in ipairs(whitelist) do
								if w == p.Name or w == p.UserId then
									found = true
									break
								end
							end
						...
						if keep and #blacklist > 0 then
							for _, b in ipairs(blacklist) do
								if b == p.Name or b == p.UserId then
									keep = false
									break
								end
							end
						...
						if keep and autoupdate.only_show_friends then
							local isFriend = false
							pcall(function()
								isFriend = LP:IsFriendsWith(p.UserId)
							end)
							if not isFriend then keep = false end
						end
						if keep and autoupdate.only_show_non_friends then
							local isFriend = false
							pcall(function()
								isFriend = LP:IsFriendsWith(p.UserId)
							end)
							if isFriend then keep = false end
						end

						if keep and autoupdate.only_show_team then
							if p.Team ~= LP.Team then keep = false end
						end
					...
					for _, p in ipairs(filteredPlayers) do
						local formattedText, hasIcon = formatPlayerText(p, Cfg.Text or "{DisplayName} (@{Username})")
						...
						Btn:SetAttribute("PlayerName", p.Name)
						Btn:SetAttribute("PlayerDisplayName", p.DisplayName)

						local labelPosX = hasIcon and 36 or 12
						local labelSizeX = hasIcon and -48 or -24

						local Label = Create("TextLabel", {
							Parent = Btn,
							Size = UDim2.new(1, labelSizeX, 1, 0),
						...
						table.insert(Library.ThemeObjects.TextDark, Label)
						...
						if hasIcon then
							...
							AddCorner(AvatarIcon, 100)
						end

						local hoverTweenBtn, leaveTweenBtn
						Btn.MouseEnter:Connect(function()
							if leaveTweenBtn then leaveTweenBtn:Cancel() end
							hoverTweenBtn = TS:Create(Btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.2})
							hoverTweenBtn:Play()
							TS:Create(Label, TweenInfo.new(0.2), {TextColor3 = SelectedTheme.Text}):Play()
						end)
						Btn.MouseLeave:Connect(function()
							if hoverTweenBtn then hoverTweenBtn:Cancel() end
							leaveTweenBtn = TS:Create(Btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.8})
							leaveTweenBtn:Play()
							TS:Create(Label, TweenInfo.new(0.2), {TextColor3 = SelectedTheme.TextDark}):Play()
						end)
					...
					local visibleCount = #filteredPlayers
					Container.CanvasSize = UDim2.new(0, 0, 0, visibleCount * 32 + 10)
				end
				...
				local connections = {}
				local autoupdate = Cfg.Autoupdate or {}
				if autoupdate.when_joined then
					table.insert(connections, Plrs.PlayerAdded:Connect(function()
						task.wait(0.5)
						RefreshList()
					end))
					table.insert(connections, Plrs.PlayerRemoving:Connect(function()
						RefreshList()
					end))
				end
			...
			Elements.AddPlayerDropDown = Elements.CreatePlayerDropdown
			...
			function Elements:CreateKeybind(Cfg)
				...
				-- Ensure CurrentKey is an Enum.KeyCode value
				if type(CurrentKey) == "string" then
					CurrentKey = Enum.KeyCode[CurrentKey] or Enum.KeyCode.RightControl
				end
						...
						if val then
							if not OnScreenBtn then
								...
								OnScreenBtn.MouseButton1Click:Connect(function()
									if dragDist < 10 then
										if Cfg.Callback then
											-- Pass Enum.KeyCode value to callback
				...
				local function Update(val)
					-- Ensure val is an Enum.KeyCode value
					if type(val) == "string" then
						val = Enum.KeyCode[val] or Enum.KeyCode.RightControl
					end
					CurrentKey = val
					Library.Flags[Flag] = val.Name
					BindBtn.Text = KeyMap[val.Name] or val.Name
					BindBtn.TextColor3 = SelectedTheme.TextDark
					Binding = false
					if OnScreenBtn then
						OnScreenBtn.Text = KeyMap[val.Name] or val.Name
					end
				end
				...
				Connection = UIS.InputBegan:Connect(function(input, gpe) 
					...
					if not Binding then 
						if input.KeyCode == CurrentKey and not gpe and not UIS:GetFocusedTextBox() then 
							...
							if Cfg.Callback then
								-- Pass Enum.KeyCode value (input.KeyCode) to callback
								Cfg.Callback(CurrentKey)
							end 
						end 
	...
	local AcrylicBlock = SettingsTab:CreateBlock({Name = "Acrylic Shader", Side = "Right"})

	AcrylicBlock:CreateToggle({
		Name = "Enable Effect",
		Flag = "Settings_Acrylic_Enabled",
		Default = Library.AcrylicSettings.Enabled,
		Callback = function(val)
			Library.AcrylicSettings.Enabled = val
		end
	})

	local materials = {"Glass", "Plastic", "SmoothPlastic", "Neon", "ForceField"}
	AcrylicBlock:CreateDropdown({
		Name = "Material",
		Flag = "Settings_Acrylic_Material",
		Items = materials,
		Default = "Glass",
		Callback = function(val)
			local mat = Enum.Material[val] or Enum.Material.Glass
			Library.AcrylicSettings.Material = mat
		end
	})

	AcrylicBlock:CreateColorPicker({
		Name = "Color",
		Flag = "Settings_Acrylic_Color",
		Default = Library.AcrylicSettings.PartColor,
		Callback = function(c)
			Library.AcrylicSettings.PartColor = c
		end
	})

	AcrylicBlock:CreateSlider({
		Name = "Acrylic Transparency",
		Flag = "Settings_Acrylic_Transparency",
		Min = 0,
		Max = 1,
		Default = Library.AcrylicSettings.PartTransparency,
		Callback = function(val)
			Library.AcrylicSettings.PartTransparency = val
		end
	})

	AcrylicBlock:CreateSlider({
		Name = "Blur Intensity (Near)",
		Flag = "Settings_Acrylic_BlurIntensity",
		Min = 0,
		Max = 10,
		Default = Library.AcrylicSettings.DepthOfField.NearIntensity,
		Callback = function(val)
			Library.AcrylicSettings.DepthOfField.NearIntensity = val
		end
	})

	AcrylicBlock:CreateSlider({
		Name = "Blur Focus Distance",
		Flag = "Settings_Acrylic_FocusDistance",
		Min = 0,
		Max = 100,
		Default = Library.AcrylicSettings.DepthOfField.FocusDistance,
		Callback = function(val)
			Library.AcrylicSettings.DepthOfField.FocusDistance = val
		end
	})
