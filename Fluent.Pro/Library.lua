...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			function SubTab:AddPlayerList(Config)
				Config = Config or {}
				local AutoUpdate = Config.AutoUpdate or false
				local Multi = Config.Multi or false
				local UseIcon = Config.UseIcon or false
				local Callback = Config.Callback or function() end

				local PlayerList = { Type = "PlayerList" }
				...
				PlayerList.Container = Container
				local ScrollFrame = Container:FindFirstChild("PlayerListScroll")
				...
				local function UpdatePlayerList()
					...
					for _, Player in pairs(Players) do
						...
						if UseIcon then
							...
							PlayerButton:FindFirstChildOfClass("UICorner").CornerRadius = UDim.new(0, 4)
							Avatar.Parent = PlayerButton

							pcall(function()
								local content, isReady = game:GetService("Players"):GetUserThumbnailAsync(Player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
						...
						local TextDisplay = New("TextLabel", {
							Name = "PlayerLabel",
							Size = UseIcon and UDim2.new(1, -40, 1, 0) or UDim2.new(1, -10, 1, 0),
							Position = UseIcon and UDim2.fromOffset(32, 0) or UDim2.fromOffset(5, 0),
							BackgroundTransparency = 1,
							TextXAlignment = Enum.TextXAlignment.Left,
							...
							TextColor3 = Color3.fromRGB(240, 240, 240),
							RichText = true,
							Text = Player.DisplayName .. " <font color=\"rgb(150, 150, 150)\">(@" .. Player.Name .. ")</font>",
							Parent = PlayerButton,
						})
						...
						local function UpdateButtonAppearance()
							if Selected then
								...
								PlayerButton.BackgroundTransparency = 0.9
							end
						end

						Creator.AddSignal(PlayerButton.MouseButton1Click, function()
							...
							if Multi then
								...
								for Frame, _ in pairs(PlayerFrames) do
									if Frame ~= PlayerButton then
										local isSelected = false
										PlayerFrames[Frame] = isSelected
						...
						Creator.AddSignal(PlayerButton.MouseEnter, function()
							if not Selected then
								PlayerButton.BackgroundTransparency = 0.8
							end
						end)

						Creator.AddSignal(PlayerButton.MouseLeave, function()
							UpdateButtonAppearance()
						end)

						PlayerFrames[PlayerButton] = Selected
						UpdateButtonAppearance()
					end
				...
				if AutoUpdate then
					Creator.AddSignal(game:GetService("Players").PlayerAdded, function()
						UpdatePlayerList()
					end)

					Creator.AddSignal(game:GetService("Players").PlayerRemoving, function()
						UpdatePlayerList()
					end)
				end
				...
				setmetatable(PlayerList, Elements)
				return PlayerList
			end
		...
		function Tab:AddPlayerList(Config)
			if self.SelectedSubTab > 0 and self.SubTabs[self.SelectedSubTab] then
				return self.SubTabs[self.SelectedSubTab]:AddPlayerList(Config)
			end
...
function Elements:Visible(Visible)
...
local InterfaceManager = {} do
	...
	function InterfaceManager:BuildInterfaceSection(tab)
		...
		if Library.UseAcrylic and not Mobile then
			...
			section:AddToggle("LiquidGlassToggle", {


				Title = "Liquid Glass",


				Description = "Switch to Liquid Glass effect instead of Acrylic",


				Default = Settings.LiquidGlass or false,


				Callback = function(Value)


					Library:ToggleLiquidGlass(Value)


					Settings.LiquidGlass = Value
...
function Library:ToggleLiquidGlass(Value)


	if Library.Window then


		Library.LiquidGlassEnabled = Value
		...
		if Value then
			...
			pcall(function()
				...
				local LiquidGlass = require(script.Parent.Parent:WaitForChild("Modules"):WaitForChild("LiquidGlass"))


				if LiquidGlass then


					Library.LiquidGlassInstance = LiquidGlass
			...
			if Library.LiquidGlassInstance then


				Library.LiquidGlassInstance = nil
